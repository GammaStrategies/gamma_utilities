...
def create_rewards_status_ramses(
    ...
    if apr_ordered_hypervisor_status_db_list := get_from_localdb(
        network=chain.database_name,
        ...
        logging.getLogger(__name__).warning(
            f" No sufficient hypervisor data found to calculate rewards apr for Ramses {hypervisor_status['address']} between {period_ini_timestamp} and {current_timestamp} on periods from {period_ini} to {current_period}"
        )
