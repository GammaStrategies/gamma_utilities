...
import logging
from apps.feeds.utils import get_reward_pool_prices
...
class hypervisor_periods_cleopatra(hypervisor_periods_base):
    def __init__(self, chain: Chain, hypervisor_status: dict, rewarder_static: dict):
        ...
        self.type = rewarderType.CLEOPATRA
    ...
    def _execute_inLoop_startItem(
        ...
        # create hypervisor
        _hypervisor_w3 = cleopatra_hypervisor(
    ...
    def _execute_postLoop(self, hypervisor_data: dict):
        ...
        if reward_apr_data_to_save := self._calculate_apr(
            ...
            reward_data = {
                ...
                "dex": Protocol.CLEOPATRA.database_name,
                "rewarder_address": self.rewarder_address,
                "rewarder_type": self.type,
                "rewarder_refIds": [],
    ...
    def _init_fill(self):
        ...
        if not self.hypervisor_status["totalSupply"]:
            logging.getLogger(__name__).debug(
                f"Can't calculate rewards status for cleopatra hype {self.hypervisor_status['symbol']} {self.hypervisor_status['address']} because it has no supply at block {self.hypervisor_status['block']}"
            )
            return []

        # create hypervisor
        hypervisor_status_w3 = cleopatra_hypervisor(
    ...
    def _calculations(
        ...
        # create hypervisor
        _temp_hype_status = cleopatra_hypervisor(
            address=hypervisor_address,
            network=network,
            block=block,
        )
    ...
    def _calculate_apr(
        ...
        try:
            ...
            for item in items_to_calc_apr:
                ...
                if not tvl:
                    logging.getLogger(__name__).warning(
                        f" using total supply to calc cleopatra reward apr because there is no staked value for {network} {hypervisor_address} using item {item}"
                    )
            ...
            logging.getLogger(__name__).exception(
                f" Error while calculating rewards yield for cleopatra hypervisor {hypervisor_address} reward token {rewardToken_address} at block {block} err: {e}"
            )
