...
class hypervisor_periods_ramses(hypervisor_periods_base):
    def __init__(self, chain: Chain, hypervisor_status: dict, rewarder_static: dict):
        """Creates ramses rewards data for the specified hypervisor status
        ...
        self.type = rewarderType.RAMSES_v2
    ...
    def _execute_inLoop_startItem(
        ...
        _hypervisor_w3 = ramses_hypervisor(
    ...
    def _init_fill(self):
        ...
        if not self.hypervisor_status["totalSupply"]:
            logging.getLogger(__name__).debug(
                f"Can't calculate rewards status for ramses hype {self.hypervisor_status['symbol']} {self.hypervisor_status['address']} because it has no supply at block {self.hypervisor_status['block']}"
            )
            return []

        # create ramses hypervisor
        hypervisor_status_w3 = ramses_hypervisor(
    ...
    def _calculate_apr(
        ...
        try:
            ...
            for item in items_to_calc_apr:
                ...
                if not tvl:
                    logging.getLogger(__name__).warning(
                        f" using total supply to calc ramses reward apr because there is no staked value for {network} {hypervisor_address} using item {item}"
                    )
