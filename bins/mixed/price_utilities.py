...
class price_scraper:
    ...
    #     )
    #     # zyberswap thegraph
    #     self.thegraph_zyberswap_connector = thegraph_utilities.zyberswap_scraper(
    #         cache=cache, cache_savePath=cache_savePath
    #     )
    #     # zyberswap thegraph
    #     self.thegraph_thena_connector = thegraph_utilities.thena_scraper(
    #         cache=cache, cache_savePath=cache_savePath
    #     )
    ...
    def init_apis(self, cache: bool, cache_savePath: str):
        ...
        self.thegraph_connectors = {
            ...
            "zyberswapv3": thegraph_utilities.quickswap_scraper(
                cache=cache, cache_savePath=cache_savePath
            ),
            "thena": thegraph_utilities.quickswap_scraper(
                cache=cache, cache_savePath=cache_savePath
            ),
        }
    ...
    def get_price(
        ...
        if _price is None or _price == 0:
            ...
            for dex, connector in thegraph_connectors.items():
                ...
                with contextlib.suppress(Exception):
                    ...
                    # logging.getLogger(LOG_NAME).debug(
                    #         f" Could not get {network}'s token {token_id} price at block {block} from {dex} subgraph."
                    #     )
        ...
        #     logging.getLogger(LOG_NAME).debug(
        #         f" Trying to get {network}'s token {token_id} price at block {block} from uniswapv3 subgraph"
        #     )
        ...
        #     except Exception:
        #         logging.getLogger(LOG_NAME).debug(
        #             f" Could not get {network}'s token {token_id} price at block {block} from uniswapv3 subgraph."
        #         )
        ...
        #     logging.getLogger(LOG_NAME).debug(
        #         f" Trying to get {network}'s token {token_id} price at block {block} from quickswap subgraph"
        #     )

        #     try:
        #         _price = self._get_price_from_anyAlgebra_thegraph(
        #             thegraph_connector=self.thegraph_quickswap_connector,
        #             network=network,
        #             token_id=token_id,
        #             block=block,
        #             of=of,
        ...
        #     except Exception:
        #         logging.getLogger(LOG_NAME).debug(
        #             f" Could not get {network}'s token {token_id} price at block {block} from quickswap subgraph."
        #         )
    ...
    #         # return zeros
    #         logging.getLogger(LOG_NAME).warning(
    #             f"one or all price variables of {network}'s token {token_symbol} (address {token_id}) at block {block} from uniswap subgraph are ZERO. Can't get price.  --> data: {_data}"
    #         )
    ...
    #                     f"Unexpected error while getting price of {network}'s token address {token_id} at block {block} from uniswap subgraph   data:{_data}      .error: {error}"
    #                 )

    #         else:
    #             logging.getLogger(LOG_NAME).error(
    #                 f"Unexpected error while getting price of {network}'s token address {token_id} at block {block} from uniswap subgraph   data:{_data}      .error: {sys.exc_info()[0]}"
    #             )

    #         _price = 0
    #     except IndexError as err:
    #         logging.getLogger(LOG_NAME).error(
    #             f"No data returned while getting price of {network}'s token address {token_id} at block {block} from uniswap subgraph   data:{_data}      .error: {sys.exc_info()[0]}"
    #         )
    ...
    #         # return zeros
    #         logging.getLogger(LOG_NAME).warning(
    #             f"one or all price variables of {network}'s token {token_symbol} (address {token_id}) at block {block} from quickswap subgraph are ZERO. Can't get price.  --> data: {_data}"
    #         )
    ...
    #             for error in _data["errors"]:
    #                 logging.getLogger(LOG_NAME).error(
    #                     f"Unexpected error while getting price of {network}'s token address {token_id} at block {block} from quickswap subgraph   data:{_data}      .error: {error}"
    #                 )

    #         else:
    #             logging.getLogger(LOG_NAME).exception(
    #                 f"Unexpected error while getting price of {network}'s token address {token_id} at block {block} from quickswap subgraph   data:{_data}      .error: {sys.exc_info()[0]}"
    #             )

    #         _price = 0
    #     except IndexError as err:
    #         logging.getLogger(LOG_NAME).error(
    #             f"No data returned while getting price of {network}'s token address {token_id} at block {block} from quickswap subgraph   data:{_data}      .error: {sys.exc_info()[0]}"
    #         )
