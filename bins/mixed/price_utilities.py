...
class price_scraper:
    ...
    def __init__(self, cache:bool=True, cache_filename:str="", cache_folderName:str=""):
        # init cache
        self.cache = cache_utilities.price_cache(cache_filename, cache_folderName) if cache else None
   ...
   ## CONFIG ##
    def init_apis(self, cache:bool, cache_savePath:str):
        # price thegraph 
        self.thegraph_univ3_connector = thegraph_utilities.uniswapv3_scraper(cache=cache,cache_savePath=cache_savePath)
        # blocks->tiumestamp thegraph
        self.thegraph_block_connector = thegraph_utilities.blocks_scraper(cache=cache,cache_savePath=cache_savePath)
        # price coingecko 
        self.coingecko_price_connector = coingecko_utilities.coingecko_price_helper()


   ## PUBLIC ##
    def get_price(self, network: str, token_id: str, block: int= 0, of:str="USD")-> tuple:
        """ 
        ...
        try:
            _price = self.cache.get_data(chain_id=network, address=token_id, block=block, key=of)
        except:
            _price = None
        ...
        if _price != None and _price != 0 and self.cache != None:
            ...
            self.cache.add_data(chain_id=network, address=token_id, block=block, key=of, data=_price, save2file=True)
        else:
            # not found
            logging.getLogger(__name__).warning(" {}'s token {} price at block {} not found".format(network, token_id, block))
        
        # return result
        return _price

   # PRIV
    def _get_price_from_univ3_thegraph(self, network:str, token_id:str, block: int, of:str)->float:
        ...
        if of == "USD":
            ...
            if block != 0:
                ...
                _data = self.thegraph_univ3_connector.get_all_results(
                        network=network, query_name="tokens", where=_where_query, block=_block_query)
            else:
        ...
        try:
            ...
            if float(_data["totalValueLockedUSD"]) > 0 and float(_data["totalValueLocked"]) > 0:
                # get unit usd price from value locked
                _price = float(_data["totalValueLockedUSD"])/float(_data["totalValueLocked"])
            elif "volume" in _data and float(_data["volume"]) > 0 and "volumeUSD" in _data and float(_data["volumeUSD"]) > 0:
    ...
    def _get_price_from_coingecko(self, network:str, token_id:str, block: int, of:str)->float:
    ...
    def _convert_block_to_timestamp(self, network:str, block:int)->int:
        try:
            block_data = self.thegraph_block_connector.get_all_results(
                    network=network, query_name="blocks", 
                    where=""" number: "{}" """.format(block))[0]
