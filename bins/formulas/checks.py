...
def checkUInt128(number):
    ...
    if number < 0 or number > MAX_UINT128:
        raise ValueError("Input must be between 0 and 2^128 - 1")


def checkInt128(number):
    checkInt(number)
    if number < MIN_INT128 or number > MAX_INT128:
        raise ValueError("Input must be between -2^127 and 2^127 - 1")


def checkInt256(number):
    checkInt(number)
    if number < MIN_INT256 or number > MAX_INT256:
        raise ValueError("Input must be between -2^255 and 2^255 - 1")


def checkUInt160(number):
    checkInt(number)
    if number < 0 or number > MAX_UINT160:
        raise ValueError("Input must be between 0 and 2^160 - 1")


def checkUInt256(number):
    checkInt(number)
    if number < 0 or number > MAX_UINT256:
        raise ValueError("Input must be between 0 and 2^256 - 1")


def checkUInt8(number):
    checkInt(number)
    if number < 0 or number > MAX_UINT8:
        raise ValueError("Input must be between 0 and 2^8 - 1")


def checkInt24(number):
    checkInt(number)
    if number < MIN_INT24 or number > MAX_INT24:
        raise ValueError("Input must be between -2^23 and 2^23 - 1")
