...
def checkUInt128(number):
    ...
    if number < 0 or number > MAX_UINT128:
        raise ValueError("Input must be between 0 and 2^128 - 1   [{number}]")


def checkInt128(number):
    checkInt(number)
    if number < MIN_INT128 or number > MAX_INT128:
        raise ValueError("Input must be between -2^127 and 2^127 - 1   [{number}]")


def checkInt256(number):
    checkInt(number)
    if number < MIN_INT256 or number > MAX_INT256:
        raise ValueError("Input must be between -2^255 and 2^255 - 1   [{number}]")


def checkUInt160(number):
    checkInt(number)
    if number < 0 or number > MAX_UINT160:
        raise ValueError("Input must be between 0 and 2^160 - 1   [{number}]")


def checkUInt256(number):
    checkInt(number)
    if number < 0 or number > MAX_UINT256:
        raise ValueError("Input must be between 0 and 2^256 - 1   [{number}]")


def checkUInt8(number):
    checkInt(number)
    if number < 0 or number > MAX_UINT8:
        raise ValueError("Input must be between 0 and 2^8 - 1   [{number}]")


def checkInt24(number):
    checkInt(number)
    if number < MIN_INT24 or number > MAX_INT24:
        raise ValueError("Input must be between -2^23 and 2^23 - 1   [{number}]")


def checkfloat(input):
    if type(input) != float:
        raise TypeError("Input must be a float   [{input}]")


def checkString(input):
    if type(input) != str:
        raise TypeError("Input must be a string   [{input}]")
...
def checkDict(input):
    if type(input) != dict:
        raise TypeError("Input must be a dict   [{input}]")
