    ...
    if os.path.exists(path_to_file):
        with open(path_to_file, 'r') as f:
            try:
                ...
                logging.getLogger(__name__).exception("Unexpected error while loading {} file    .error: {}".format(path_to_file, sys.exc_info()[0]))
    return None
...
def save_json(filename:str, data, folder_path:str)->bool:
    ...
    with open(path_to_tempfile, 'w') as f:
        try:
            ...
            logging.getLogger(__name__).exception("Unexpected error while saving {} file    .error: {}".format(path_to_tempfile, sys.exc_info()[0]))
            return False
        # remove old file
        try:
            ...
            logging.getLogger(__name__).exception("Unexpected error while deleting {} file    .error: {}".format(path_to_file, sys.exc_info()[0]))

        # rename tmp file
        try:
            os.rename(path_to_tempfile, path_to_file)
        except Exception as e:
            logging.getLogger(__name__).exception("Unexpected error while renaming {} file    .error: {}".format(path_to_tempfile, sys.exc_info()[0]))
            return False
