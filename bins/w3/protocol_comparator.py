...
class comparator_v1():
   ...
   # SETUP
    def __init__(self, configuration:dict, protocol:str):
        ...
        # create price helper
        self.price_helper = price_utilities.price_scraper(cache=self.configuration["cache"]["enabled"], cache_filename="uniswapv3_price_cache", cache_folderName=self.configuration["cache"]["save_path"])


   # PUBLIC
    def create_collection(self, addresses:list, network:str, progress_callback=None)->dict:
        ...
        if progress_callback:
            progress_callback(text=" defining {} {}'s initial and end block to scrape".format(self.protocol, network))
        ...
        data = self.get_dwrfts(addresses=addresses, network=network, 
                                block_ini=block_ini, block_end=block_end, 
                                progress_callback=progress_callback, max_blocks=10000)
    ...
    def create_static(self, address:str, block:int, network:str)->dict:
        ...
        # Web3 helper
        web3_helper = self.create_web3_helper(address=address, web3Provider=self.create_web3_provider(network=network))
        # define block
        web3_helper.block = block if block != 0 else web3_helper.w3.eth.get_block("latest").number
        ...
        if web3_helper.token0.address.lower() == "0x9f8F72aA9304c8B593d555F12eF6589cC3A579A2".lower():
            result["name_token0"] = "MKV"
        else:
            result["name_token0"] = web3_helper.token0.symbol
        if web3_helper.token1.address.lower() == "0x9f8F72aA9304c8B593d555F12eF6589cC3A579A2".lower():
    ...
    def create_status(self, address:str, block:int, network:str, address_token0=None, address_token1=None)->dict:
        ...
        # Web3 helper
        web3_helper = self.create_web3_helper(address=address, web3Provider=self.create_web3_provider(network=network), block=block)
        ...
        result["price_usd_token0"] =  self.price_helper.get_price(network=network, token_id=address_token0, block=block, of="USD")
        result["price_usd_token1"] =  self.price_helper.get_price(network=network, token_id=address_token1, block=block, of="USD")
        
        # set usd total values
        result["fees"]["uncollected_total_in_usd"] = (result["fees"]["uncollected_token0"]*result["price_usd_token0"]) + (result["fees"]["uncollected_token1"]*result["price_usd_token1"])
        result["fees"]["owed_total_in_usd"] = (result["fees"]["owed_token0"]*result["price_usd_token0"]) + (result["fees"]["owed_token1"]*result["price_usd_token1"])

        return result

    def create_status_ofBlocks(self, address:str, network:str, block_list:list, progress_callback=None)->dict:
        """  
        ...
        args = ((address, block, network, address_token0, address_token1) for block in block_list)
        # threaded loop
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as ex:
            for result_item in ex.map(lambda p: self.create_status(*p), args):
                ...
                rem_progress -= 1
                if progress_callback:
                    progress_callback(text=" getting {} {}'s {}-{} status at block {}".format(self.protocol, network, address_token0[-4:], address_token1[-4:], result_item["block"]), remaining=rem_progress, total=tot_progress)
    
        # return result
        return result

    def create_status_autoblocks(self, address:str, network:str, progress_callback=None)->dict:
    ...
    def create_status_fromOPS(self, hypervisor:dict, progress_callback=None)->dict:
        ...
        block_list = list(set([x["block"] for x in hypervisor["deposits"]]+[x["block"] for x in hypervisor["withdraws"]]+[x["block"] for x in hypervisor["fees"]]+[x["block"] for x in hypervisor["rebalances"]]))
        ...
        if "force_timeframe" in filters and "start_time" in filters["force_timeframe"] and "end_time" in filters["force_timeframe"]:
            ...
            # filter blocks
            block_list = [x for x in block_list if x>=block_list_dates[0] and x<=block_list_dates[-1]]
        ...
        # return result
        return self.create_status_ofBlocks(address=address, network=network, block_list=block_list, progress_callback=progress_callback)
    ...
    def populate_all_status(self, hypervisor:dict, progress_callback=None):
        ...
        for block in hypervisor["status"].keys():
            ...
            hypervisor["status"][block]["deposits"] = template(oftype="hypervisor")["status"]["deposits"]
            hypervisor["status"][block]["cost"] = template(oftype="hypervisor")["status"]["cost"]
            hypervisor["status"][block]["withdraws"] = template(oftype="hypervisor")["status"]["withdraws"]
            hypervisor["status"][block]["rebalances"] = template(oftype="hypervisor")["status"]["rebalances"]

             # there are fees fields to be preserved. ( uncollected and owed fees ) 
            if not "fees" in hypervisor["status"][block]:
                hypervisor["status"][block]["fees"] = template(oftype="hypervisor")["status"]["fees"]
            ...
            if "deposits" in hypervisor:
                ...
                for x in hypervisor["deposits"]:
                    ...
                    if x["block"] <= block:
                        ...
                        hypervisor["status"][block]["deposits"]["aggregated_qtty_token0"] += x["qtty_token0"]
                        hypervisor["status"][block]["deposits"]["aggregated_qtty_token1"] += x["qtty_token1"]

                        
                        # udate first
                        if not "first_deposit" in hypervisor["status"][block] or hypervisor["status"][block]["first_deposit"] == None:
                            hypervisor["status"][block]["first_deposit"] = x["timestamp"]
                        hypervisor["status"][block]["first_deposit"] = min(hypervisor["status"][block]["first_deposit"], x["timestamp"])

                        # update shares
                        hypervisor["status"][block]["shares"] += x["shares"]

                        # update cost
                        hypervisor["status"][block]["cost"]["qtty_token0"] += x["qtty_token0"]
                        hypervisor["status"][block]["cost"]["qtty_token1"] += x["qtty_token1"]
                        
                        # totals conversions
                        if x["price_usd_token0"] != 0 and x["price_usd_token1"] != 0:
                            # deposits
                            hypervisor["status"][block]["deposits"]["aggregated_total_in_token0"] += x["qtty_token0"] + (x["qtty_token1"]*(x["price_usd_token1"]/x["price_usd_token0"]))
                            hypervisor["status"][block]["deposits"]["aggregated_total_in_token1"] += x["qtty_token1"] + (x["qtty_token0"]*(x["price_usd_token0"]/x["price_usd_token1"]))
                            hypervisor["status"][block]["deposits"]["aggregated_total_in_usd"] += (x["qtty_token0"]*x["price_usd_token0"]) + (x["qtty_token1"]*x["price_usd_token1"])
                            # cost
                            hypervisor["status"][block]["cost"]["qtty_total_in_usd"] += (x["qtty_token0"]*x["price_usd_token0"]) + (x["qtty_token1"]*x["price_usd_token1"])
                            hypervisor["status"][block]["cost"]["qtty_total_in_token0"] += x["qtty_token0"] + (x["qtty_token1"]*(x["price_usd_token1"]/x["price_usd_token0"]))
                            hypervisor["status"][block]["cost"]["qtty_total_in_token1"] += x["qtty_token1"] + (x["qtty_token0"]*(x["price_usd_token0"]/x["price_usd_token1"]))
                        else:
                            # TODO: log at least
                            pass
                    ...
                    if progress_callback:
                        try:
                            progress_callback(text=" populating {} {}'s {} status-> deposits".format(self.protocol, hypervisor["static"]["network"], hypervisor["static"]["name"], block), remaining=rem_progress, total=tot_progress)
                        except:
                            pass
            ...
            if "withdraws" in hypervisor:
                ...
                for x in hypervisor["withdraws"]:
                    # check if inside boundaries
                    if x["block"] <= block:
                        hypervisor["status"][block]["withdraws"]["aggregated_qtty_token0"] += x["qtty_token0"]
                        hypervisor["status"][block]["withdraws"]["aggregated_qtty_token1"] += x["qtty_token1"]

                        # update last
                        if not "last_withdraw" in hypervisor["status"][block] or hypervisor["status"][block]["last_withdraw"] == None:
                            hypervisor["status"][block]["last_withdraw"] = x["timestamp"]
                        hypervisor["status"][block]["last_withdraw"] = max(hypervisor["status"][block]["last_withdraw"], x["timestamp"])
                        ...
                        if x["price_usd_token0"] != 0 and x["price_usd_token1"] != 0:
                            # withdraws
                            hypervisor["status"][block]["withdraws"]["aggregated_total_in_token0"] += x["qtty_token0"] + (x["qtty_token1"]*(x["price_usd_token1"]/x["price_usd_token0"]))
                            hypervisor["status"][block]["withdraws"]["aggregated_total_in_token1"] += x["qtty_token1"] + (x["qtty_token0"]*(x["price_usd_token0"]/x["price_usd_token1"]))
                            hypervisor["status"][block]["withdraws"]["aggregated_total_in_usd"] += (x["qtty_token0"]*x["price_usd_token0"]) + (x["qtty_token1"]*x["price_usd_token1"])
                            # cost
                            hypervisor["status"][block]["cost"]["qtty_total_in_usd"] -= (x["qtty_token0"]*x["price_usd_token0"]) + (x["qtty_token1"]*x["price_usd_token1"])
                            hypervisor["status"][block]["cost"]["qtty_total_in_token0"] -= x["qtty_token0"] + (x["qtty_token1"]*(x["price_usd_token1"]/x["price_usd_token0"]))
                            hypervisor["status"][block]["cost"]["qtty_total_in_token1"] -= x["qtty_token1"] + (x["qtty_token0"]*(x["price_usd_token0"]/x["price_usd_token1"]))
                    ...
                    if progress_callback:
                        try:
                            progress_callback(text=" populating {} {}'s {} status-> withdraws".format(self.protocol, hypervisor["static"]["network"], hypervisor["static"]["name"], block), remaining=rem_progress, total=tot_progress)
                        except:
                            pass
            ...
            if "rebalances" in hypervisor:
                ...
                for x in hypervisor["rebalances"]:
                    if x["block"] <= block:
                        # add qtty
                        hypervisor["status"][block]["rebalances"]["aggregated_qtty_total"] += 1

                        if not "last_rebalance" in hypervisor["status"][block] or hypervisor["status"][block]["last_rebalance"] == None:
                            hypervisor["status"][block]["last_rebalance"] = x["timestamp"]
                        hypervisor["status"][block]["last_rebalance"] = max(hypervisor["status"][block]["last_rebalance"], x["timestamp"])

                    # progress
                    rem_progress -= 1
                    if progress_callback:
                        try:
                            progress_callback(text=" populating {} {}'s {} status-> rebalances".format(self.protocol, hypervisor["static"]["network"], hypervisor["static"]["name"], block), remaining=rem_progress, total=tot_progress)
                        except:
                            pass
            ...
            if "fees" in hypervisor:
                ...
                for x in hypervisor["fees"]:
                    if x["block"] <= block:
                        hypervisor["status"][block]["fees"]["aggregated_qtty_token0"] += x["gross_token0"]
                        hypervisor["status"][block]["fees"]["aggregated_qtty_token1"] += x["gross_token1"]

                        # totals conversions
                        if x["price_usd_token0"] != 0 and x["price_usd_token1"] != 0:
                            # fees
                            hypervisor["status"][block]["fees"]["aggregated_total_in_token0"] += x["gross_token0"] + (x["gross_token1"]*(x["price_usd_token1"]/x["price_usd_token0"]))
                            hypervisor["status"][block]["fees"]["aggregated_total_in_token1"] += x["gross_token1"] + (x["gross_token0"]*(x["price_usd_token0"]/x["price_usd_token1"]))
                            hypervisor["status"][block]["fees"]["aggregated_total_in_usd"] += (x["gross_token0"]*x["price_usd_token0"]) + (x["gross_token1"]*x["price_usd_token1"])
                    ...
                    if progress_callback:
                        try:
                            progress_callback(text=" populating {} {}'s {} status-> fees".format(self.protocol, hypervisor["static"]["network"], hypervisor["static"]["name"], block), remaining=rem_progress, total=tot_progress)
                        except:
                            pass
            
            
            # add uncollected owed fees to totals
            hypervisor["status"][block]["fees"]["aggregated_qtty_token0"] += hypervisor["status"][block]["fees"]["uncollected_token0"]+hypervisor["status"][block]["fees"]["owed_token0"]
            hypervisor["status"][block]["fees"]["aggregated_qtty_token1"] += hypervisor["status"][block]["fees"]["uncollected_token1"]+hypervisor["status"][block]["fees"]["owed_token1"]
            # totals conversions
            if hypervisor["status"][block]["price_usd_token0"] != 0 and hypervisor["status"][block]["price_usd_token1"] != 0:
                # fees
                hypervisor["status"][block]["fees"]["aggregated_total_in_token0"] += (hypervisor["status"][block]["fees"]["uncollected_token0"]+hypervisor["status"][block]["fees"]["owed_token0"]) + ((hypervisor["status"][block]["fees"]["uncollected_token1"]+hypervisor["status"][block]["fees"]["owed_token1"])*(hypervisor["status"][block]["price_usd_token1"]/hypervisor["status"][block]["price_usd_token0"]))
                hypervisor["status"][block]["fees"]["aggregated_total_in_token1"] += (hypervisor["status"][block]["fees"]["uncollected_token1"]+hypervisor["status"][block]["fees"]["owed_token1"]) + ((hypervisor["status"][block]["fees"]["uncollected_token0"]+hypervisor["status"][block]["fees"]["owed_token0"])*(hypervisor["status"][block]["price_usd_token0"]/hypervisor["status"][block]["price_usd_token1"]))
                hypervisor["status"][block]["fees"]["aggregated_total_in_usd"] += ((hypervisor["status"][block]["fees"]["uncollected_token0"]+hypervisor["status"][block]["fees"]["owed_token0"])*hypervisor["status"][block]["price_usd_token0"]) + ((hypervisor["status"][block]["fees"]["uncollected_token1"]+hypervisor["status"][block]["fees"]["owed_token1"])*hypervisor["status"][block]["price_usd_token1"])
                        
    def create_chart(self, hypervisor:dict, forced_startime=None, progress_callback=None)->dict:
        ...
        # define startime
        startime = (dt.datetime.utcnow()-dt.timedelta(days=365*150) ) if forced_startime == None else forced_startime
        ...
        for block,status in hypervisor["status"].items():
            ...
            # progress
            progress_callback(text=" creating {} {}'s {} hypervisor chart at block {}".format(protocol, network, hypervisor_id, block), remaining=rem_progress, total=tot_progress)
            ...
            if dt.datetime.fromtimestamp(status["timestamp"]) > startime:
                ...
                # fill row
                chart_datarow["datetime"] = dt.datetime.fromtimestamp(status["timestamp"])
                ...
                chart_datarow["days_passed"] = (chart_datarow["datetime"]-control_vars["first_datetime"]).total_seconds()/(60*60*24)
                ...
                chart_datarow["tvl_total_in_usd"] = status["qtty_token0"]* status["price_usd_token0"] + status["qtty_token1"]* status["price_usd_token1"]

                # supply
                chart_datarow["total_supply"] = status["supply"]

                # Fees
                chart_datarow["aggregatedFees_qtty_token0"] = status["fees"]["aggregated_qtty_token0"]
                chart_datarow["aggregatedFees_qtty_token1"] = status["fees"]["aggregated_qtty_token1"]
                chart_datarow["aggregatedFees_total_in_usd"] = status["fees"]["aggregated_total_in_usd"]

                # deposits & withdraws
                chart_datarow["aggregatedDeposits_token0"] = status["deposits"]["aggregated_qtty_token0"]
                chart_datarow["aggregatedDeposits_token1"] = status["deposits"]["aggregated_qtty_token1"]
                chart_datarow["aggregatedDeposits_total_in_usd"] = status["deposits"]["aggregated_total_in_usd"]
                chart_datarow["aggregatedWithdraws_token0"] = status["withdraws"]["aggregated_qtty_token0"]
                chart_datarow["aggregatedWithdraws_token1"] = status["withdraws"]["aggregated_qtty_token1"]
                chart_datarow["aggregatedWithdraws_total_in_usd"] = status["withdraws"]["aggregated_total_in_usd"]


                ### CALCULATED FIELDS ###

                # Share price
                chart_datarow["price_share_usd"] = (chart_datarow["tvl_total_in_usd"]/chart_datarow["total_supply"]) if chart_datarow["total_supply"] > 0 else 0
                #   price Share percentage change
                control_vars["aggregated_price_per_share"] += 0 if control_vars["last_chart_datarow"]["price_share_usd"] == 0 else (chart_datarow["price_share_usd"]-control_vars["last_chart_datarow"]["price_share_usd"])/control_vars["last_chart_datarow"]["price_share_usd"]
                chart_datarow["price_share_change"] = control_vars["aggregated_price_per_share"]
                ...
                if len(control_vars["last_status"].keys()) > 0:
                    ...
                    if status["fees"]["aggregated_qtty_token0"] == 0 and control_vars["last_status"]["fees"]["aggregated_qtty_token0"] > 0:
                    ...
                    # fee % fields
                    if status["fees"]["uncollected_token0"] > 0 or status["fees"]["uncollected_token1"] > 0 :
                        # calculate feeAPR from last 
                        fees_period_token0 = status["fees"]["uncollected_token0"] + status["fees"]["owed_token0"] - control_vars["last_status"]["fees"]["uncollected_token0"] - control_vars["last_status"]["fees"]["owed_token0"]
                        fees_period_token1 = status["fees"]["uncollected_token1"] + status["fees"]["owed_token1"] - control_vars["last_status"]["fees"]["uncollected_token1"] - control_vars["last_status"]["fees"]["owed_token1"]
                        fees_period_usd = status["fees"]["uncollected_total_in_usd"] + status["fees"]["owed_total_in_usd"] - control_vars["last_status"]["fees"]["uncollected_total_in_usd"] - control_vars["last_status"]["fees"]["owed_total_in_usd"]

                        chart_datarow["fees_period_timepassed"] = status["timestamp"] - control_vars["last_status"]["timestamp"]
                        chart_datarow["fees_period_generated_token0"] = fees_period_token0
                        chart_datarow["fees_period_generated_token1"] = fees_period_token1
                        chart_datarow["fees_period_generated_usd"] = fees_period_usd

                        chart_datarow["fees_period_start_TVL_token0"] = control_vars["last_status"]["qtty_token0"]
                        chart_datarow["fees_period_start_TVL_token1"] = control_vars["last_status"]["qtty_token1"]
                        chart_datarow["fees_period_start_TVL_usd"] = control_vars["last_chart_datarow"]["tvl_total_in_usd"]

                        chart_datarow["fees_period_end_TVL_token0"] = status["qtty_token0"]
                        chart_datarow["fees_period_end_TVL_token1"] = status["qtty_token1"]
                        chart_datarow["fees_period_end_TVL_usd"] = chart_datarow["tvl_total_in_usd"]
   ...
   # HELPERS
    def create_web3_provider(self, network:str)->Web3:
        ...
        w3 = Web3(Web3.HTTPProvider(self.configuration["sources"]["web3Providers"][network], request_kwargs={'timeout': 60})) 
    ...
    def create_web3_helper(self, address:str, web3Provider:Web3, block:int=0):
        ...
        if self.protocol == "gamma":
            return onchain_utilities.gamma_hypervisor_cached(address=address, web3Provider=web3Provider, block=block)

        elif self.protocol == "arrakis":
            return onchain_utilities.arrakis_hypervisor_cached(address=address, web3Provider=web3Provider, block=block)

        else:
            raise ValueError(" No web3 helper defined for {} protocol".format(self.protocol))
        ...
        if self.protocol == "gamma":
            result = onchain_utilities.data_collector(
                        ...
                        topics_data_decoders={
                            ...
                            "gamma_rebalance":  ['int24', 'uint256', 'uint256', 'uint256', 'uint256', 'uint256'],
            ...
            result = onchain_utilities.data_collector(
                        ...
                        topics_data_decoders={
                            ...
                            "arrakis_deposit": ['address', 'uint256', 'uint256', 'uint256', 'uint128'],
                            "arrakis_withdraw": ['address','uint256', 'uint256', 'uint256', 'uint128'],
    ...
    def get_dwrfts(self, addresses:list, network:str, block_ini:int, block_end:int, progress_callback=None, max_blocks=10000)->dict:
        ...
        dta_coll.get_all_operations(
            block_ini=block_ini,
            block_end=block_end,
            contracts=[Web3.toChecksumAddress(x) for x in addresses], max_blocks=max_blocks)
        ...
        for k,v in dta_coll._data.items():
            result[k] = self.convert_dtaCollector_item(address=k, data_item=v, network=network, progress_callback=progress_callback)

        # return result
        return result
    
    def convert_dtaCollector_item(self, address:str, data_item, network, progress_callback=None):
        ...
        w3helper = self.create_web3_helper(address=address,web3Provider=self.create_web3_provider(network=network))
            ...
            if key in _defs.keys():
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as ex:
                    for result_item in ex.map(lambda p: _defs[key](*p), args):
                        ...
                        _remaining -= 1
                        if progress_callback:
                            progress_callback(text="processing {}-{} {} ".format(address_token0, address_token1, key), remaining=_remaining, total=_totals)
                        
                        # get price
                        result_item["price_usd_token0"] = self.price_helper.get_price(network=network, token_id=address_token0, block=result_item["block"], of="USD")
                        result_item["price_usd_token1"] = self.price_helper.get_price(network=network, token_id=address_token1, block=result_item["block"], of="USD")
    ...
    def get_standard_blockBounds(self, network:str)->tuple:
        ...
        dummy_helper = self.create_web3_helper(address="0x0000000000000000000000000000000000000000", web3Provider=self.create_web3_provider(network=network))
        ...
        if "force_timeframe" in filters.keys():
            try:    
                ...
                block_ini = dummy_helper.blockNumberFromTimestamp(timestamp=start_timestamp, inexact_mode="after", eq_timestamp_position="first")

                # return result
                return block_ini,block_end

            except:
                logging.getLogger(__name__).exception(" Unexpected error calc. {}'s {} force_timeframe block scan option     .error: {}".format(self.protocol, network, sys.exc_info()[0]))
        ...
        secs = dummy_helper.average_blockTime(blocksaway=dummy_helper._w3.eth.get_block('latest').number*0.85)
    ...
    def get_networkScan_blockNumbers(self, network:str)->tuple:
        ...
        current_folder = os.path.join(output["files"]["save_path"], self.protocol, network)
        ...
        hypervisor_files = sorted(Path(current_folder).iterdir(), key=os.path.getmtime, reverse=False) if os.path.isdir(current_folder) else []
        ...
        if hypervisor_files != None:
            for hyp_file in hypervisor_files:
                ...
                t_last_block_scraped = max([
                    max( [x["blockNumber"] for x in hyp_file["deposits"]]) if "deposits" in hyp_file else block_ini,
                    max( [x["blockNumber"] for x in hyp_file["withdraws"]]) if "withdraws" in hyp_file else block_ini,
                    max( [x["blockNumber"] for x in hyp_file["rebalances"]]) if "rebalances" in hyp_file else block_ini,
                    max( [x["blockNumber"] for x in hyp_file["fees"]]) if "fees" in hyp_file else block_ini,
                    max([x["blockNumber"] for x in hyp_file["transactions"]]) if "transactions" in hyp_file else block_ini
                    ])
                
                # set global last block scraped ( min of all hypervisors)
                block_ini = min([block_ini,t_last_block_scraped]) if block_ini != 0 else t_last_block_scraped

        # return result
        return block_ini,block_end

    def get_blocklist_fromDates(self, date_ini:dt.datetime, date_end:dt.datetime, network:str)->list:
        ...
        block_ini = dummy_helper.blockNumberFromTimestamp(timestamp=dt.datetime.timestamp(date_ini), inexact_mode="after", eq_timestamp_position="first")
        block_end = dummy_helper.blockNumberFromTimestamp(timestamp=dt.datetime.timestamp(date_end), inexact_mode="before", eq_timestamp_position="last")
    ...
    def get_custom_blockBounds(self, date_ini:dt.datetime, date_end:dt.datetime, network:str, step="week")->tuple:
        ...
        if step == "week":    
            ...
            result_date_ini = dt.datetime(year=date_ini.year,month=date_ini.month,day=date_ini.day,hour=0,minute=0,second=0)

            # convert date_end in that same week last day last hour
            result_date_end = dt.datetime(year=date_end.year,month=date_end.month,day=date_end.day,hour=23,minute=59,second=59)

            step_secs = (60*60*24)
        else:
            raise NotImplementedError(" blockBounds step not implemented: {}".format(step))
        ...
        # force seek block numbers from datetime
        block_ini = dummy_helper.blockNumberFromTimestamp(timestamp=dt.datetime.timestamp(result_date_ini), inexact_mode="after", eq_timestamp_position="first")
        block_end = dummy_helper.blockNumberFromTimestamp(timestamp=dt.datetime.timestamp(result_date_end), inexact_mode="before", eq_timestamp_position="last")
