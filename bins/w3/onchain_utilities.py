...
class web3wrap():
   ...
   # SETUP
    ...
    def setup_cache(self):
        ...
        # create cache helper
        self._cache = cache_utilities.standard_property_cache(filename=cache_filename, folder_name="data/cache/onchain", reset=False)
   ...
   # HELPERS
    ...
    def blockNumberFromTimestamp(self, timestamp:dt.datetime.timestamp, inexact_mode="before", eq_timestamp_position="first")->int:
        """ Will 
        ...
        while block_curr.timestamp != timestamp:
            ...
            if (dt.datetime.utcnow() - _startime).total_seconds() > (15):
                if inexact_mode == "before":
                    ...
                    raise ValueError(" Inexact method chosen is not valid:->  {}".format(inexact_mode))
                # exit loop
                break
        ...
        # log result
        if found_exact:
            logging.getLogger(__name__).debug(" Took {} on-chain queries to find block number {} of timestamp {}".format(queries_cost, block_curr.number, timestamp))
        else:
            logging.getLogger(__name__).warning(" Could not find the exact block number from timestamp -> took {} on-chain queries to find block number {} ({}) closest to timestamp {}  -> original-found difference {}".format(queries_cost, block_curr.number, block_curr.timestamp, timestamp, timestamp-block_curr.timestamp))
...
class data_collector():
   ...
   # SETUP
    ...
    _w3 = None
    _data = dict() # all data retrieved will be saved here. { <contract_address>: {<topic>: <topic defined content> } }
    ...
    def __init__(self, topics:dict, topics_data_decoders:dict, web3Provider:Web3=None, web3Provider_url:str=""):
        ...
        # define helper
        self._web3_helper = erc20(address="0x0000000000000000000000000000000000000000",web3Provider=self._w3)
   ...
   # PUBLIC
    def get_all_operations(self, block_ini:int, block_end:int, contracts:list, 
            topics:dict={}, topics_data_decoders:dict={}, max_blocks:int=5000):
        ...
        for event in self._web3_helper.get_chunked_events(eventfilter={'fromBlock': block_ini,
                                                'toBlock': block_end,
                                                'address': contracts,
            ...
            if self._progress_callback:
                self._progress_callback(text="processing {} at block:{}".format(topic, event.blockNumber), remaining=block_end-event.blockNumber, total=block_end-block_ini)
   ...
   # HELPERS
    def _save_topic(self, topic:str, event, data):
        ...
        decimals_contract = self._token_helpers[itm["address"].lower()]["decimals_contract"]
...
class gamma_hypervisor(erc20):
   ...
   # PROPERTIES
    ...
    def getTotalAmounts(self)->dict:
        ...
        result = {"total0":tmp[0]/(10**self.token0.decimals),
...
class arrakis_hypervisor(erc20):
   ...
   # PROPERTIES
    ...
    def gelatoBalance0(self)->int:
        return self._contract.functions.gelatoBalance0().call(block_identifier=self.block)

    @property
    def gelatoBalance1(self)->int:
        return self._contract.functions.gelatoBalance1().call(block_identifier=self.block)
    ...
    def gelatoRebalanceBPS(self)->int:
        return self._contract.functions.gelatoRebalanceBPS().call(block_identifier=self.block)

    @property
    def gelatoSlippageBPS(self)->int:
        return self._contract.functions.gelatoSlippageBPS().call(block_identifier=self.block)

    @property
    def gelatoSlippageInterval(self)->int:
        return self._contract.functions.gelatoSlippageInterval().call(block_identifier=self.block)
    
    @property
    def gelatoWithdrawBPS(self)->int:
        return self._contract.functions.gelatoWithdrawBPS().call(block_identifier=self.block)
    ...
    def getMintAmounts(self, amount0Max, amount1Max)->dict:
        ...
        tmp = self._contract.functions.getMintAmounts(amount0Max, amount1Max).call(block_identifier=self.block)
        return {"amount0":tmp[0],
                "amount1":tmp[1],
                "mintAmount":tmp[2]
         }
    ...
    def getPositionID(self)->str:
        return self._contract.functions.getPositionID().call(block_identifier=self.block)
    ...
    def getUnderlyingBalances(self)->dict:
        """ getUnderlyingBalances _summary_
         ...
         """
        tmp = self._contract.functions.getUnderlyingBalances().call(block_identifier=self.block)
        return  {"amount0Current":tmp[0],
    ...
    def getUnderlyingBalancesAtPrice(self, sqrtRatioX96)->dict:
        """ 
         ...
         """      
        tmp = self._contract.functions.getUnderlyingBalancesAtPrice(sqrtRatioX96).call(block_identifier=self.block)
        return {"amount0Current":tmp[0],
                "amount1Current":tmp[1],
         }
    ...
    def managerBalance0(self)->int:
        return self._contract.functions.managerBalance0().call(block_identifier=self.block)

    @property
    def managerBalance1(self)->int:
        return self._contract.functions.managerBalance1().call(block_identifier=self.block)
    
    @property
    def managerFeeBPS(self)->int:
        return self._contract.functions.managerFeeBPS().call(block_identifier=self.block)
    
    @property
    def managerTreasury(self)->str:
        return self._contract.functions.managerTreasury().call(block_identifier=self.block)
