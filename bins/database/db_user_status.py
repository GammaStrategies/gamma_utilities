...
class user_status_hypervisor_builder:
    ...
    def _process_operations(self):
        ...
        operations_to_process = self._mix_operations_status()
    ...
    def _process_deposit(self, operation: dict) -> user_status:
        ...
        new_user_status = self._add_globals_to_user_status(
            current_operation=new_user_status,
    ...
    def _add_globals_to_user_status(
        ...
        current_operation.last_underlying_token0 = last_operation.underlying_token0
        current_operation.last_underlying_token1 = last_operation.underlying_token1
        current_operation.last_total_underlying_in_usd = (
            last_operation.total_underlying_in_usd
        )
        current_operation.last_total_underlying_in_token0 = (
            last_operation.total_underlying_in_token0
        )
        current_operation.last_total_underlying_in_token1 = (
            last_operation.total_underlying_in_token1
        )

        # get hypervisor status at block
        current_status_data = self.get_hypervisor_status(block=current_operation.block)
        if len(current_status_data) == 0:
            raise ValueError(
                f" No hypervisor status found for {self.network}'s {self.address} at block {current_operation.block}"
            )
        ...
        total_shares = (
            self.total_shares(
                block=current_operation.block,
                logIndex=current_operation.logIndex,
                exclude_address=current_operation.address,
            )
            + current_operation.shares_qtty
        )
    ...
    def _share_fees_with_acounts(self, operation: dict):
        ...
        with concurrent.futures.ThreadPoolExecutor() as ex:
            for result_status, result_user_share in ex.map(
                lambda p: loop_share_fees(*p), args
            ):
                # check inconsistencies
                # if (ctrl_total_percentage_applied + result_user_share) > 1:
                #     logging.getLogger(__name__).warning(
                #         " The total percentage applied while calc. user fees exceeds 100% at {}'s {} hype {} -> {}".format(
                #             self.network,
                #             self.protocol,
