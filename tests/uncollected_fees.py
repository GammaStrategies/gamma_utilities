...
def test_uncollected_fees_comparison_api_onchain(configuration:dict, threaded:bool=False):
    ...
    csv_filename = "{}_test_uncollected_fees_comparison_api_onchain{}.csv".format(network, "_threaded" if threaded else "")
    ...
    def do_loop_work(hyp_id, hypervisor, web3Provider):
        ...
        if hypervisor["totalFees0"] == 0 and onchain_totalFees0>0:
            ...
            difference_token0 = ((hypervisor["totalFees0"]-onchain_totalFees0)/hypervisor["totalFees0"]) if hypervisor["totalFees0"] != 0 else 0
        
        if hypervisor["totalFees1"] == 0 and onchain_totalFees1>0:
            difference_token1 = 1
        else:
            difference_token1 = ((hypervisor["totalFees1"]-onchain_totalFees1)/hypervisor["totalFees1"]) if hypervisor["totalFees1"] != 0 else 0
    ...
    official_uncollected_data = net_utilities.get_request(official_api_url) # data collected from official endpoint api originaly sourced from subgraph 
    ...
    with tqdm.tqdm(total=len(official_uncollected_data.keys())) as progress_bar:
        ...
        if not threaded:
            for hyp_id, hypervisor in official_uncollected_data.items():
                progress_bar.set_description("processing {}'s {}".format(network, hypervisor["symbol"]))
            ...
            # threaded
            args = ((hyp_id, hypervisor, web3Provider) for hyp_id, hypervisor in official_uncollected_data.items())
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as ex:
                for result_item in ex.map(lambda p: do_loop_work(*p), args):
                    # progress
                    progress_bar.set_description("processed {}'s {}".format(network, result_item["symbol"]))
...
def test_uncollected_fees_comparison_api_thegraph(configuration:dict, threaded:bool=False):
    ...
    csv_filename = "{}_test_uncollected_fees_comparison_api_thegraph{}.csv".format(network,"_threaded" if threaded else "")
    csv_filename = os.path.join(CURRENT_FOLDER,csv_filename)
    ...
    class tick():
        ...
        def __init__(self, value:int, feeGrowthOutside0X128:int, feeGrowthOutside1X128:int):
    ...
    def do_loop_work(hyp_id, hypervisor, gamma_helper, uniswapv3_helper):
        ...
        error = False
        subgraph_totalFees0 = subgraph_totalFees1 = difference_token0 = difference_token1 = 0
        try:
            # get hypervisor's data from thegraph
            thegraph_hyperv = gamma_helper.get_all_results(network=network, query_name="uniswapV3Hypervisors", where=""" id: "{}" """.format(hyp_id))[0]

            # get uniswap pool and tick info  [ feeGrowthGlobal0X128 and feeGrowthGlobal1X128]
            univ3_pool = uniswapv3_helper.get_all_results(network=network, query_name="pools", where=""" id: "{}" """.format(thegraph_hyperv["pool"]["id"]))[0]
            ...
            univ3_ticks = uniswapv3_helper.get_all_results(network=network, query_name="ticks", where=""" tickIdx_in: {}, poolAddress: "{}" """.format(ticks, thegraph_hyperv["pool"]["id"]))

            # classify univ3 ticks in a dict
            feeGrowthOutside = dict()
            for tk in univ3_ticks:
                if not tk["tickIdx"] in feeGrowthOutside:
                    feeGrowthOutside[int(tk["tickIdx"])] = tick(int(tk["tickIdx"]),int(tk["feeGrowthOutside0X128"]),int(tk["feeGrowthOutside1X128"]))
            ...
            try:
                feeGrowthOutsideLower = feeGrowthOutside[_lowerTick].feeGrowthOutside0X128
            except:
                error=True
                # log error
                logging.getLogger(__name__).error("{} tick {} is not present in uniswap's subgraph using <tickIdx_in: {}, poolAddress:{} > in the query. ".format(hypervisor["symbol"], _lowerTick, ticks, thegraph_hyperv["pool"]["id"]))
                # skip 
                feeGrowthOutsideLower = 0
                logging.getLogger(__name__).warning("{} token0 base position's feeGrowthOutsideLower of token0 has been set to zero due to errors encountered   hyp_id:{}".format(hypervisor["symbol"], thegraph_hyperv["id"]))
            try:
                feeGrowthOutsideUpper = feeGrowthOutside[_upperTick].feeGrowthOutside0X128
            except:
                error=True
                # log error
                logging.getLogger(__name__).error("{} tick {} is not present in uniswap's subgraph using <tickIdx_in: {}, poolAddress:{} > in the query.".format(hypervisor["symbol"], _lowerTick, ticks, thegraph_hyperv["pool"]["id"]))
                ...
                logging.getLogger(__name__).warning("{} token0 base position's feeGrowthOutsideUpper has been set to zero due to errors encountered   hyp_id:{}".format(hypervisor["symbol"], thegraph_hyperv["id"]))
            ...
            base_fees0 = univ3_formulas.get_uncollected_fees(
                            ...
                            feeGrowthInsideLast= int(thegraph_hyperv["baseFeeGrowthInside0LastX128"]),
            ...
            feeGrowthOutsideLower = feeGrowthOutsideUpper = 0
            try:
                feeGrowthOutsideLower = feeGrowthOutside[_lowerTick].feeGrowthOutside1X128
            except:
                error=True
                # log error
                logging.getLogger(__name__).error("{} tick {} is not present in uniswap's subgraph using <tickIdx_in: {}, poolAddress:{} > in the query. ".format(hypervisor["symbol"], _lowerTick, ticks, thegraph_hyperv["pool"]["id"]))
                # skip 
                feeGrowthOutsideLower = 0
                logging.getLogger(__name__).warning("{} token1 base position's feeGrowthOutsideLower of token0 has been set to zero due to errors encountered   hyp_id:{}".format(hypervisor["symbol"], thegraph_hyperv["id"]))
            try:
                feeGrowthOutsideUpper = feeGrowthOutside[_upperTick].feeGrowthOutside1X128
            except:
                error=True
                # log error
                logging.getLogger(__name__).error("{} tick {} is not present in uniswap's subgraph using <tickIdx_in: {}, poolAddress:{} > in the query.".format(hypervisor["symbol"], _lowerTick, ticks, thegraph_hyperv["pool"]["id"]))
                # skip 
                feeGrowthOutsideUpper = 0
                logging.getLogger(__name__).warning("{} token1 base position's feeGrowthOutsideUpper has been set to zero due to errors encountered   hyp_id:{}".format(hypervisor["symbol"], thegraph_hyperv["id"]))
            
            base_fees1 = univ3_formulas.get_uncollected_fees(
                            feeGrowthGlobal = int(univ3_pool["feeGrowthGlobal1X128"]),
                            feeGrowthOutsideLower= feeGrowthOutsideLower,
                            feeGrowthOutsideUpper= feeGrowthOutsideUpper,
                            feeGrowthInsideLast=int(thegraph_hyperv["baseFeeGrowthInside1LastX128"]),
                            tickCurrent=currentTick,
                            liquidity=liquidity,
                            tickLower=_lowerTick,
                            tickUpper=_upperTick
            ...
            try:
                ...
                # skip 
                feeGrowthOutsideLower = 0
                logging.getLogger(__name__).warning("{} token0 limit position's feeGrowthOutsideLower of token0 has been set to zero due to errors encountered   hyp_id:{}".format(hypervisor["symbol"], thegraph_hyperv["id"]))
            try:
                feeGrowthOutsideUpper = feeGrowthOutside[_upperTick].feeGrowthOutside0X128
            except:
                error=True
            ...
            limit_fees0 = univ3_formulas.get_uncollected_fees(
                            ...
                            feeGrowthOutsideLower=feeGrowthOutsideLower,
                            feeGrowthOutsideUpper=feeGrowthOutsideUpper,
                            feeGrowthInsideLast=int(thegraph_hyperv["limitFeeGrowthInside0LastX128"]),
                            tickCurrent=currentTick,
                            liquidity=liquidity,
                            tickLower=_lowerTick,
                            tickUpper=_upperTick
            ...
            limit_fees1 = univ3_formulas.get_uncollected_fees(
                            ...
                            feeGrowthOutsideLower=feeGrowthOutsideLower,
                            feeGrowthOutsideUpper=feeGrowthOutsideUpper,
                            feeGrowthInsideLast=int(thegraph_hyperv["limitFeeGrowthInside1LastX128"]),
                            tickCurrent=currentTick,
                            liquidity=liquidity,
                            tickLower=_lowerTick,
                            tickUpper=_upperTick
            ...
            # define totalfees
            subgraph_totalFees0 = base_fees0 + limit_fees0 + thegraph_hyperv["baseTokensOwed0"] + thegraph_hyperv["limitTokensOwed0"]
            subgraph_totalFees1 = base_fees1 + limit_fees1 + thegraph_hyperv["baseTokensOwed1"] + thegraph_hyperv["limitTokensOwed1"]

            # differences situations
            if hypervisor["totalFees0"] == 0 and subgraph_totalFees0>0:
                difference_token0 = 1
            else:
                difference_token0 = ((hypervisor["totalFees0"]-subgraph_totalFees0)/hypervisor["totalFees0"]) if hypervisor["totalFees0"] != 0 else 0
            
            if hypervisor["totalFees1"] == 0 and subgraph_totalFees1>0:
                difference_token1 = 1
            else:
                difference_token1 = ((hypervisor["totalFees1"]-subgraph_totalFees1)/hypervisor["totalFees1"]) if hypervisor["totalFees1"] != 0 else 0

        except:
            logging.getLogger(__name__).exception(" Unexpected error     .error: {}".format( sys.exc_info()[0]))
            error = True
    ...
    with tqdm.tqdm(total=len(official_uncollected_data.keys())) as progress_bar:
        ...
        if not threaded:
            ...
            for hyp_id, hypervisor in official_uncollected_data.items():
                ...
                progress_bar.set_description("processed {}'s {}".format(network, hypervisor["symbol"]))
                
                # add data to result
                result.append( do_loop_work(hyp_id,hypervisor,gamma_helper,uniswapv3_helper))
            
                # update progress
                progress_bar.update(1)
        else:
            # threaded
            args = ((hyp_id,hypervisor,gamma_helper,uniswapv3_helper) for hyp_id, hypervisor in official_uncollected_data.items())
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as ex:
                for result_item in ex.map(lambda p: do_loop_work(*p), args):
                    # progress
                    progress_bar.set_description("processed {}'s {}".format(network, result_item["symbol"]))
...
# passed
# equal 100%
def test_uncollected_fees_comparison_formulas_thegraph(configuration:dict, threaded:bool=False):
    ...
    csv_filename = "{}_test_uncollected_fees_comparison_formulas_thegraph{}.csv".format(network,"_threaded" if threaded else "")
    csv_filename = os.path.join(CURRENT_FOLDER,csv_filename)
    ...
    def do_loop_work(hypervisor:dict, gamma_helper, uniswapv3_helper):
        ...
        try:
            # get uniswap pool and tick info  [ feeGrowthGlobal0X128 and feeGrowthGlobal1X128]
            univ3_pool = uniswapv3_helper.get_all_results(network=network, query_name="pools", where=""" id: "{}" """.format(hypervisor["pool"]["id"]))[0]
            ...
            # get all ticks [feeGrowthOutside1X128 and feeGrowthOutside0X128]
            univ3_ticks = uniswapv3_helper.get_all_results(network=network, query_name="ticks", where=""" tickIdx_in: {}, poolAddress: "{}" """.format(ticks, hypervisor["pool"]["id"]))
            ...
            try:
                feeGrowthOutsideLower_token0 = feeGrowthOutside[_lowerTick].feeGrowthOutside0X128
            except:
                error = True
                # log error
                logging.getLogger(__name__).error("{} tick {} is not present in uniswap's subgraph using <tickIdx_in: {}, poolAddress:{} > in the query. ".format(hypervisor_name, _lowerTick, ticks, hypervisor["pool"]["id"]))
                # skip 
                feeGrowthOutsideLower_token0 = 0
                logging.getLogger(__name__).warning("{} token0 base position's feeGrowthOutsideLower of token0 has been set to zero due to errors encountered   hyp_id:{}".format(hypervisor_name, hypervisor["id"]))
            try:
                feeGrowthOutsideUpper_token0 = feeGrowthOutside[_upperTick].feeGrowthOutside0X128
            except:
                error = True
                # log error
                logging.getLogger(__name__).error("{} tick {} is not present in uniswap's subgraph using <tickIdx_in: {}, poolAddress:{} > in the query.".format(hypervisor_name, _lowerTick, ticks, hypervisor["pool"]["id"]))
                ...
                logging.getLogger(__name__).warning("{} token0 base position's feeGrowthOutsideUpper has been set to zero due to errors encountered   hyp_id:{}".format(hypervisor_name, hypervisor["id"]))
            
            # define token1 feeGrowthOutsideLower & Upper vars
            feeGrowthOutsideLower_token1 = feeGrowthOutsideUpper_token1 = 0
            try:
                feeGrowthOutsideLower_token1 = feeGrowthOutside[_lowerTick].feeGrowthOutside1X128
            except:
                error = True
                # log error
                logging.getLogger(__name__).error("{} tick {} is not present in uniswap's subgraph using <tickIdx_in: {}, poolAddress:{} > in the query. ".format(hypervisor_name, _lowerTick, ticks, hypervisor["pool"]["id"]))
                ...
                logging.getLogger(__name__).warning("{} token1 base position's feeGrowthOutsideLower of token0 has been set to zero due to errors encountered   hyp_id:{}".format(hypervisor_name, hypervisor["id"]))
            try:
                feeGrowthOutsideUpper_token1 = feeGrowthOutside[_upperTick].feeGrowthOutside1X128
            except:
                error = True
                # log error
                logging.getLogger(__name__).error("{} tick {} is not present in uniswap's subgraph using <tickIdx_in: {}, poolAddress:{} > in the query.".format(hypervisor_name, _lowerTick, ticks, hypervisor["pool"]["id"]))
                ...
                logging.getLogger(__name__).warning("{} token1 base position's feeGrowthOutsideUpper has been set to zero due to errors encountered   hyp_id:{}".format(hypervisor_name, hypervisor["id"]))
            ...
            # gammawire
            gammawireFormula_base_fees0,gammawireFormula_base_fees1 = univ3_formulas.get_uncollected_fees_vGammawire(
            ...
            try:
                ...
                # skip 
                feeGrowthOutsideLower_token0 = 0
                logging.getLogger(__name__).warning("{} token0 limit position's feeGrowthOutsideLower of token0 has been set to zero due to errors encountered   hyp_id:{}".format(hypervisor_name, hypervisor["id"]))
            try:
                feeGrowthOutsideUpper_token0 = feeGrowthOutside[_upperTick].feeGrowthOutside0X128
            except:
                error = True
                ...
                # skip 
                feeGrowthOutsideUpper_token0 = 0
                logging.getLogger(__name__).warning("{} token0 limit position's feeGrowthOutsideUpper has been set to zero due to errors encountered   hyp_id:{}".format(hypervisor_name, hypervisor["id"]))
            ...
            try:
                ...
                # skip 
                feeGrowthOutsideLower_token1 = 0
                logging.getLogger(__name__).warning("{} token1 limit position's feeGrowthOutsideLower of token0 has been set to zero due to errors encountered   hyp_id:{}".format(hypervisor_name, hypervisor["id"]))
            try:
                feeGrowthOutsideUpper_token1 = feeGrowthOutside[_upperTick].feeGrowthOutside1X128
            except:
                error = True
                ...
                # skip 
                feeGrowthOutsideUpper_token1 = 0
                logging.getLogger(__name__).warning("{} token1 limit position's feeGrowthOutsideUpper has been set to zero due to errors encountered   hyp_id:{}".format(hypervisor_name, hypervisor["id"]))
            ...
            # gammawire
            gammawireFormula_limit_fees0,gammawireFormula_limit_fees1 = univ3_formulas.get_uncollected_fees_vGammawire(
                fee_growth_global_0= int(univ3_pool["feeGrowthGlobal0X128"]), 
                fee_growth_global_1= int(univ3_pool["feeGrowthGlobal1X128"]),
                tick_current= currentTick, 
                tick_lower= _lowerTick, 
            ...
            # define totalfees
            gammawire_totalFees0 = gammawireFormula_base_fees0 + gammawireFormula_limit_fees0 + hypervisor["baseTokensOwed0"] + hypervisor["limitTokensOwed0"] 
            gammawire_totalFees1 = gammawireFormula_base_fees1 + gammawireFormula_limit_fees1 + hypervisor["baseTokensOwed1"] + hypervisor["limitTokensOwed1"] 
            
            alternative_totalFees0 = base_fees0 + limit_fees0 + hypervisor["baseTokensOwed0"] + hypervisor["limitTokensOwed0"]
            alternative_totalFees1 = base_fees1 + limit_fees1 + hypervisor["baseTokensOwed1"] + hypervisor["limitTokensOwed1"]

            # differences situations
            if gammawire_totalFees0 == 0 and alternative_totalFees0>0:
                difference_token0 = 1
            else:
                difference_token0 = ((gammawire_totalFees0-alternative_totalFees0)/gammawire_totalFees0) if gammawire_totalFees0 != 0 else 0
            
            if gammawire_totalFees1 == 0 and alternative_totalFees1>0:
                difference_token1 = 1
            else:
                difference_token1 = ((gammawire_totalFees1-alternative_totalFees1)/gammawire_totalFees1) if gammawire_totalFees1 != 0 else 0

        except:
            logging.getLogger(__name__).exception(" Unexpected error  while processing {}       .error: {}".format( hypervisor_name, sys.exc_info()[0]))
            error = True
    ...
    hypervisors = gamma_helper.get_all_results(network=network, query_name="uniswapV3Hypervisors")
    ...
    with tqdm.tqdm(total=len(hypervisors)) as progress_bar:
        ...
        if not threaded:
            ...
            for hypervisor in hypervisors:
                ...
                progress_bar.set_description("processed {}'s {}".format(network, hypervisor_name))
            ...
            # threaded
            args = ((hypervisor,gamma_helper,uniswapv3_helper) for hypervisor in hypervisors)
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as ex:
                for result_item in ex.map(lambda p: do_loop_work(*p), args):
                    # progress
...
# passed
# equal 100%
def test_uncollected_fees_comparison_formulas_onchain(configuration:dict, threaded:bool=False):
    """ Comparison between uncollected fees formula used at gammawire.net endpoint and 
    ...
    network = "ethereum"
    web3Provider_url = configuration["sources"]["web3Providers"][network]
    csv_filename = "{}_test_uncollected_fees_comparison_formulas_onchain{}.csv".format(network,"_threaded" if threaded else "")
    csv_filename = os.path.join(CURRENT_FOLDER,csv_filename)
    ...
    def do_loop_work(hyp_id, web3Provider):
        ...
        # setup helper
        gamma_web3Helper = onchain_utilities.gamma_hypervisor_cached(address=hyp_id, web3Provider=web3Provider)
        ...
        pool_position_base = gamma_web3Helper.pool.position(ownerAddress=Web3.toChecksumAddress(gamma_web3Helper.address.lower()),
        ...
        pool_position_limit = gamma_web3Helper.pool.position(ownerAddress=Web3.toChecksumAddress(gamma_web3Helper.address.lower()),
        ...
        # BASE
        # gammawire formula
        gammawire_base_fees0,gammawire_base_fees1 = univ3_formulas.get_uncollected_fees_vGammawire(
        ...
        # LIMIT
        # gammawire formula
        gammawire_limit_fees0,gammawire_limit_fees1 = univ3_formulas.get_uncollected_fees_vGammawire(
            fee_growth_global_0= feeGrowthGlobal0X128, 
            fee_growth_global_1= feeGrowthGlobal1X128,
            tick_current= tickCurrent, 
    ...
    # get a list of hypervisors from thegraph
    hypervisors = gamma_helper.get_all_results(network=network, query_name="simple_uniswapV3Hypervisors")
    ...
    with tqdm.tqdm(total=len(hypervisors)) as progress_bar:
        ...
        if not threaded:
            ...
            for hypervisor in hypervisors:
                ...
                progress_bar.set_description("processed {}'s {}".format(network, hypervisor["id"]))
