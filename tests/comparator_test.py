...
def test_comparator_phase1():
    ...
    for protocol in protocols:
        ...
        for network in list(networks.keys()):
            ...
            # apply filters
            if (
                "hypervisors_included" in filters.keys()
                and network in filters["hypervisors_included"].keys()
            ):
            ...
            with tqdm.tqdm(total=100) as progress_bar:
                ...
                try:
                    ...
                    for mkey in main_frame.keys():
                        main_frame[mkey]["status"] = {
                            int(k): v for k, v in main_frame[mkey]["status"].items()
                        }
                    ...
                    if len(main_frame.keys()) != len(addresses):
                        ...
                        addresses_left = general_utilities.differences(
                            list(main_frame.keys()), addresses
                        )
                        # scrape addresses left
                        partial_frame = helper.create_collection(
                            addresses=addresses_left,
                            network=network,
                            progress_callback=_update_progress,
                        )
                    ...
                    logging.getLogger(__name__).exception(
                        " Unexpected error loading json information for {}'s {}      .error: {}".format(
                            protocol, network, sys.exc_info()[0]
                        )
                    )
                ...
                for hypervisor_id, hypervisor in main_frame.items():
                    ...
                    try:
                        # progress
                        _update_progress(
                            text=" getting {} {}'s {} hypervisor static info".format(
                                protocol, network, hypervisor_id
                            ),
                            remaining=rem_progress,
                            total=tot_progress,
                        )

                        hypervisor["static"] = helper.create_static(
                            address=hypervisor_id, block=0, network=network
                        )
                    except:
                        logging.getLogger(__name__).exception(
                            " Unexpected error creating static information for {}'s {} hypervisor {}      .error: {}".format(
                                protocol, network, hypervisor_id, sys.exc_info()[0]
                            )
                        )
                    ...
                    try:
                        # progress
                        _update_progress(
                            text=" getting {} {}'s {} hypervisor status info".format(
                                protocol, network, hypervisor_id
                            ),
                            remaining=rem_progress,
                            total=tot_progress,
                        ...
                        hypervisor["status"] = helper.create_status_fromOPS(
                            hypervisor=hypervisor, progress_callback=_update_progress
                        )
                    except:
                        logging.getLogger(__name__).exception(
                            " Unexpected error creating historic status information for {}'s {} hypervisor {}      .error: {}".format(
                                protocol, network, hypervisor_id, sys.exc_info()[0]
                            )
                        )
                    ...
                    try:
                        # progress
                        _update_progress(
                            text=" getting {} {}'s {} hypervisor status extra info".format(
                                protocol, network, hypervisor_id
                            ),
                            remaining=rem_progress,
                            total=tot_progress,
                        ...
                        helper.populate_all_status(
                            hypervisor=hypervisor, progress_callback=_update_progress
                        )
                    except:
                        logging.getLogger(__name__).exception(
                            " Unexpected error populating all status complementary information for {}'s {} hypervisor {}      .error: {}".format(
                                protocol, network, hypervisor_id, sys.exc_info()[0]
                            )
                        )
                    ...
                    try:
                        # progress
                        _update_progress(
                            text=" creating {} {}'s {} hypervisor chart".format(
                                protocol, network, hypervisor_id
                            ),
                            remaining=rem_progress,
                            total=tot_progress,
                        ...
                        hypervisor["chart"] = helper.create_chart(
                            hypervisor=hypervisor, progress_callback=_update_progress
                        )
                    except:
                        logging.getLogger(__name__).exception(
                            " Unexpected error building chart data for {}'s {} hypervisor {}      .error: {}".format(
                                protocol, network, hypervisor_id, sys.exc_info()[0]
                            )
                        )
...
def load_files(
    folder_path: str, addresses: list, network: str, progress_callback=None
) -> dict:
    ...
    file_list = sorted(
        [
            ...
            if (f.is_file() and f.name.startswith("0x"))
        ],
    ...
    for file in file_list:
        ...
        if len(_addresses) == 0 or fname.lower() in _addresses:
            ...
            tmp_data = file_utilities.load_json(
                filename=file.name.split(".")[0], folder_path=folder_path
            )
